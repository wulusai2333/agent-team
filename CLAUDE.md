...
当会话即将结束（用户说"下班了""明天继续""另一个 Agent 接手"），**自动生成交接备忘录**，写入 `.scratch/handoff/{date}.md`：
...
- **正在执行的技能**：{/plan-java / /tdd-java / /code-review-java / /guardrails-java}
- **当前阶段**：{步骤 N / 阶段 X}
- **目标 Issue**：{#N 或 "无"}

## 文件状态

- 未提交的更改：{git status --short 摘要}
- 当前分支：{git branch --show-current}
- 上次提交：{git log -1 --oneline}

## 已验证的内容

- 已通过的测试：{列表或 "无"}
- 已生成的文档：{CONTEXT.md / ADR / PRD / Issue 列表}
...
**规则**：
- 用户不需要主动要求——当检测到"下班""明天""切换""接手"等关键词时自动生成
- 备忘录写入后不提交（它在 `.scratch/` 下，通常被 `.gitignore`）
- 如果 `.scratch/handoff/` 目录不存在，创建它
